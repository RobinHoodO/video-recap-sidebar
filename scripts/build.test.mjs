...
test('development output cannot overwrite the installed production extension', () => {
  ...
  assert.equal(JSON.parse(prod.stdout).outDir, join(fixture, 'Thrivbe-AI/builds/video-recap-sidebar/dist'));
  const preview = run(['--input-type=module', '-e', configRunner, 'serve', 'preview']);
  assert.equal(preview.status, 0, preview.stderr);
  assert.equal(JSON.parse(preview.stdout).outDir, join(fixture, 'Thrivbe-AI/builds/video-recap-sidebar/dist'));
});
